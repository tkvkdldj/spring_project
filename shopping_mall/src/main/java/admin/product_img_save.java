...
public class product_img_save {
	...
	//컨트롤러에서 현재 실행
	public void save_imgs(MultipartFile imgs[], HttpServletRequest req) throws Exception{
		...
		while(w < imgs.length) {
			...
			if((imgs[w].getSize() > 0)) { //파일이 있으면 (첨부파일 용량 제한은 프론트에서)
				nm = imgs[w].getOriginalFilename(); //파일명을 가져와
				...
				//사본이름으로 이미지 저장
				FileCopyUtils.copy(imgs[w].getBytes(), new File(url + renm));
			}
